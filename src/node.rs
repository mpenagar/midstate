...
impl Node {
    pub fn new(data_dir: PathBuf, is_mining: bool, our_addr: SocketAddr) -> Result<Self> {
        ...
        tracing::info!(
            "Loaded state: height={} depth={} coins={}",
            state.height,
            state.depth,
            state.coins.len()
        );
    ...
    pub async fn listen(&mut self, bind_addr: SocketAddr) -> Result<()> {
        ...
        let broadcast_tx = self.peer_manager.broadcast_tx.clone();
    ...
    fn handle_new_transaction(&mut self, tx: Transaction) -> Result<()> {
        ...
        tracing::info!("Received transaction: inputs={}", inputs.join(", "));
    ...
    async fn try_mine(&mut self) -> Result<()> {
        ...
        tracing::info!("Mined batch! height={}", self.state.height);
